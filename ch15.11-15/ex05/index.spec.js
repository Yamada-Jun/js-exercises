...
test.describe("simple todo app", () => {
    ...
    test("add new todo", async ({ page }) => {
        ...
        await page.reload();    //リロードする

        expect(await countToDos(page)).toBe(1);

        const todo_reload = queryToDo(page, 0);
        const label_reload = todo_reload.getByText("質問表に質問を記載する");
        await expect(label_reload).toBeVisible();
        await expect(label_reload).toHaveCSS("text-decoration-line", "none");
    });
    ...
    test("add multiple todos", async ({ page }) => {
        ...
        const todo2_reload = queryToDo(page, 0);
        const label2_reload = todo2_reload.getByText("質問表に質問を記載する");
        await expect(label2_reload).toBeVisible();
        await expect(label2_reload).toHaveCSS("text-decoration-line", "none");

        const todo1_reload = queryToDo(page, 1);
        const label1_reload = todo1_reload.getByText("練習問題を完了する");
        await expect(label1_reload).toBeVisible();
        await expect(label1_reload).toHaveCSS("text-decoration-line", "none");
    });
    ...
    test("delete todo", async ({ page }) => {
        ...
        const todo_reload = queryToDo(page, 0);
        const label_reload = todo_reload.getByText("練習問題を完了する");
        await expect(label_reload).toBeVisible();
        await expect(label_reload).toHaveCSS("text-decoration-line", "none");
    });
    ...
    test("complete todo", async ({ page }) => {
        ...
        const todo1_reload = queryToDo(page, 0);
        const label1_reload = todo1_reload.getByText("質問表に質問を記載する");
        await expect(label1_reload).toBeVisible();
        await expect(label1_reload).toHaveCSS("text-decoration-line", "none");

        const todo2_reload = queryToDo(page, 1);
        const label2_reload = todo2_reload.getByText("練習問題を完了する");
        await expect(label2_reload).toBeVisible();
        await expect(label2_reload).toHaveCSS("text-decoration-line", "line-through");
